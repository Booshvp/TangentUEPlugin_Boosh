...
void UEngineHelper::GetSelectedName(FString& name, bool fullName)
{
	...
	if (fullName && _selectedComponent)
	{
		...
		actorName = _selectedActor->GetActorLabel();
	...
	else if (_selectedActor)
	{
		...
		name = _selectedActor->GetActorLabel();
	}

	UE_LOG(LogTemp, Display, TEXT("GetSelectedName: %s"), *name);
...
void UEngineHelper::ExclusivelySelectNamedObject(const FString &targetName)
{
	...
	if (world)
	{
		...
		while (levelIterator)
		{
			...
			// search for the actor by display label rather than internal object name, since names stored via GetActorLabel()
			AActor* actor = nullptr;
			for (AActor* levelActor : level->Actors)
			{
				if (levelActor && IsValid(levelActor) && !levelActor->IsA(AWorldSettings::StaticClass()) && levelActor->GetActorLabel() == actorName)
				{
					actor = levelActor;
					break;
				}
